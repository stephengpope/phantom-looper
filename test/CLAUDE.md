...
| `phase4.test.ts` | the Git Fixer's loop, `verifyResolved`, `POST /workspaces create=true` and `GET /github/repos` (paged, `added` marks, 404 without a token) against the fake GitHub (real paths), and AUTO-PUSH on real git: clean push, push-twice-no-self-conflict, conflict through the fixer, fixer failure leaves the branch, base-moved retry, nothing-to-push, commit-message fallback; AUTO-PULL on the same origins: dirty tree committed + base merged + branch pushed + `arrived`/`files`, nothing-behind = `clean` with no commit minted, conflict through the fixer (base untouched), fixer failure aborts and keeps the commit, and the WIRE — `/git/auto-pull` 503 unwired, then core `autoPullSession` + `codingGitTools` (plan mode drops it) + the Telegram Assistant's kit over `injectFetch` |
