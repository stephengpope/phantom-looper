...
The other direction is auto-pull: on demand, a session's branch takes the base branch in — fetch, commit the session's in-flight work, merge base in, the coding agent resolves any conflict, the branch is pushed as a backup. A pull merges rather than replays, because it lands nothing. Nothing reaches the base branch from a pull. It is how a session catches up with work others have landed before its own auto-push.
