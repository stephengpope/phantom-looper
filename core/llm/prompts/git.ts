...
Every session works on its own branch, cut from the base branch, one branch from start to finish. Nothing pushes in the background and nothing merges on a timer unless the workspace's "instant sync" setting is on (below).
...
Instant sync is a per-workspace setting (off by default) that fires both for you: while a session's container is running, a file change auto-pushes once the files have been quiet for the push debounce (30 seconds by default), and the base branch is fetched every pull interval (5 seconds by default) and auto-pulled when it moved. It runs whether or not a turn is running, and it never resolves a conflict itself: a conflict is left in place with its markers, and the coding agent is told what came in and which files conflict at its next turn, so it can resolve them and continue the rebase. It is meant for a notes or second-brain repo shared across devices; the debounce and interval are global settings a workspace may override.
